using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using BUS;

namespace DAL
{
    public class SqlFriendDAL : BUS.IFriendDAL
    {

        private ContactWebDBDataContext _db = null;

        private ContactWebDBDataContext DB
        {
            get
            {
                if (_db == null)
                    _db = new ContactWebDBDataContext(
                            System.Configuration.ConfigurationManager.ConnectionStrings["ContactWebDB"].ConnectionString);
                return _db;
            }
        }



        public IQueryable<BUS.Friend> Friends
        {
            get 
            {
                return DB.Friends.Select(dbFriend => new BUS.Friend { ContactId1 = dbFriend.ContactId1, ContactId2 = dbFriend.ContactId2});
                    
            }
        }


        public void Save(BUS.Contact contact, List<BUS.Friend> friends)
        {
            var toDelete = DB.Friends.Where(f => f.ContactId1 == contact.Id || f.ContactId2 == contact.Id);
            DB.Friends.DeleteAllOnSubmit(toDelete);
            foreach (var friend in friends)
            {
                DB.Friends.InsertOnSubmit(new Friend { ContactId1 = friend.ContactId1, ContactId2 = friend.ContactId2 });
            }

            DB.SubmitChanges();
        }
    }
}
